...
module.exports = class BreadcrumbsView {
  constructor(pane, { config, registry, treeView }) {
    ...
    this.cursorRefreshPromise = null;
    this.generation = 0;
  ...
  setItem(item) {
    ...
    this.editorSubscriptions = null;
    this.editor = editor;
    this.cursorPosition = editor?.getLastCursor().getBufferPosition() ?? null;
    this.symbolTree = null;
    this.cursorRefreshPromise = null;
    ...
    if (editor) {
      this.editorSubscriptions = new CompositeDisposable(
        ...
        editor.onDidChangeCursorPosition(({ cursor, newBufferPosition }) => {
          ...
          if (cached !== null && cached !== undefined) this.symbolTree = cached;
          if (this.symbolTree === null) this.refreshSymbolsForCursor();
          this.render();
        }),
      );
      const cached = this.registry?.peekFileSymbolTree?.(editor);
  ...
  refreshSymbolsForCursor() {
    if (this.cursorRefreshPromise) return;
    const promise = this.refreshSymbols();
    const pending = promise.finally(() => {
      if (this.cursorRefreshPromise === pending) this.cursorRefreshPromise = null;
    });
    this.cursorRefreshPromise = pending;
  }
  ...
  symbolSegments() {
    ...
    const position = this.cursorPosition ?? this.editor.getLastCursor().getBufferPosition();
