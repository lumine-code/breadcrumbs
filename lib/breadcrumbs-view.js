...
module.exports = class BreadcrumbsView {
  ...
  createPathCrumb(segment) {
    ...
    if (clickable) {
      element.type = "button";
      element.title = `Reveal ${segment.path} in the tree view`;
  ...
  createSymbolCrumb(symbol) {
    ...
    element.title = kind ? `${symbol.name} (${kind})` : symbol.name;
    element.addEventListener("click", () => this.revealSymbol(symbol));
    if (this.config?.icons !== false && symbol.icon) {
      const icon = document.createElement("span");
      icon.className = `breadcrumbs-icon icon ${symbol.icon}`;
      element.appendChild(icon);
    } else {
      this.appendIcon(element, { kind, context: "breadcrumbs" });
    }
  ...
  appendIcon(element, target) {
    if (this.config?.icons === false || (!target.path && !target.kind)) return;
    const icon = document.createElement("span");
